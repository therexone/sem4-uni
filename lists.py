...
while choice != 8:
    ...
    if choice == 1:
        ...
        even_list.append(val) if (val.isdigit() and parity(int(val))) else (odd_list.append(val) if (val.isdigit() and not parity(int(val))) else 'false')
    elif choice == 2:
        ...
        minum = min(odd_list) if min(odd_list) < min(even_list) else min(even_list)
        print('Minumum number is ', minum)
    elif choice == 6:
        maxum = max(odd_list) if max(odd_list) > max(even_list) else max(even_list)
        print('Maximum Number is: ', maxum)
    elif choice == 7 :
        index = int(input('Enter index to update:'))
        if len(raw_list) > index:
            raw_list.insert(int(input('Enter element to update')))